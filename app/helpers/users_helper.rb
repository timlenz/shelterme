module UsersHelper
  ...
  def avatar_for(user, options = {size: 40})
    ...
    if user.avatar.blank?
      image_tag("blankProfile.png", alt: user.name, class: "avatar", width: "#{size}", height: "#{size}")
    else 
      cloudinary_url = user.avatar.to_s.gsub!(/upload/,"upload/a_exif,c_thumb,h_#{size},g_faces,w_#{size}")
      image_tag(cloudinary_url, alt: user.name, class: "avatar", width: "#{size}", height: "#{size}")
    end
  end
end